...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store user information and JWT token in localStorage for session management [cite: 58, 61]
      localStorage.setItem("userInfo", JSON.stringify(data));
      
      setLoading(false);
      navigate("/"); // Redirect to the landing page or dashboard upon success 
    } catch (err) {
      setLoading(false);
      // Capture and display error messages from the backend [cite: 58, 61]
